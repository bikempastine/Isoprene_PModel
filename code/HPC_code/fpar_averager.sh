...
echo "Processing fpar folder: $YEAR"

### Loop over all netCDF files in the folder specified in the data folder
for x in $HOME/average_fpar_full/$YEAR/*.nc; 
    do
        ...
        out_file="$HOME/average_fpar_full/sandbox/${YEAR}_mean_"$(basename $data)

        # Calculate the monthly mean of the file using CDO and select values below 1
    	#cdo -b F32 -fldmean -selvalue,-inf,1 $data $out_file
	cdo timmean -setvrange,0,1 $data $out_file
...
echo "Done averaging fpar for $YEAR"

# concatenate all of the monthly mean files using CDO and save the 
# result to a single file in the results folder
cdo cat $HOME/average_fpar_full/sandbox/${YEAR}_mean_*.nc $HOME/average_fpar_full/monthly/fpar_${YEAR}_output.nc

# remove all the temporary, monthly mean files
rm  $HOME/average_fpar_full/sandbox/${YEAR}_mean*.nc

...
for x in $HOME/average_Tair_full/$YEAR/*.nc; 
    do
        ...
        out_file="$HOME/average_Tair_full/sandbox/${YEAR}_mean_"$(basename $data)

        # Calculate the monthly mean of the file using CDO
        #cdo -b F32 -ymonmean $data $out_file
	cdo timmean -setvrange,0,330 $data $out_file
...
echo "Done averaging for $YEAR"

# concatenate all of the monthly mean files using CDO and save the 
# result to a single file in the results folder
cdo cat $HOME/average_Tair_full/sandbox/${YEAR}_mean_*.nc $HOME/average_Tair_full/monthly/Tair_${YEAR}_output.nc

# remove all the temporary, monthly mean files
rm  $HOME/average_Tair_full/sandbox/${YEAR}_mean*.nc

...
for number in $(seq 0 144)
do
    python isoprene_model_HPC.py $number
done
...
for folder in "${folders[@]}"; do
    qsub -N job_SW_${folder} -o $HOME/averager_messages/messages_SW/${folder}.out -e $HOME/averager_messages/messages_SW/${folder}.err -v "YEAR=${folder}" $HOME/averager_sc$
done
